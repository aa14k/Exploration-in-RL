...
class UCRL_VTR(object):
    ...
    def compute_Q(self,s,a,k,h):
        ...
        self.Q[s,a] = env.R[(s,a)][0] + np.dot(self.X[h,:],self.theta) + np.sqrt(self.Beta(k)) \ 
            * np.sqrt(np.dot(np.dot(np.transpose(self.X[h,:]),np.linalg.inv(self.M)),self.X[h,:]))
    ...
    def compute_Qend(self,k,h):
        ...
        for s in range(env.nState):
            for a in range(env.nAction):
                #Here env.R[(s,a)][0] is the true reward from the environment
                self.Q[s,a] = env.R[(s,a)][0] + np.dot(self.X[h,:],self.theta) + np.sqrt(self.Beta(k))                     * np.sqrt(np.dot(np.dot(np.transpose(self.X[h,:]),np.linalg.inv(self.M)),self.X[h,:]))
            self.V[s] = max(self.Q[s,:])
